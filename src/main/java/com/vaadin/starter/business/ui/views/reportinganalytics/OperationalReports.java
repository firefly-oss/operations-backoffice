...
public class OperationalReports extends ViewFrame {
    ...
    private Grid<Report> createReportsGrid() {
        ...
        Collection<Report> reports = generateMockReports();
    ...
    private Collection<Report> generateMockReports() {
        ...
        String[] descriptions = {
            "This report provides a comprehensive summary of all transactions processed within the specified time period, categorized by transaction type, channel, and status.",
            "Detailed analysis of customer activity including logins, transactions, service requests, and other interactions with the system.",
            "Overview of all account operations including openings, closings, modifications, and status changes.",
            "Summary of all loan disbursements including amount, recipient, purpose, and processing time.",
            "Statistics on card issuance including card type, customer segment, delivery method, and processing time.",
            "Key performance metrics for system components including response times, availability, and error rates.",
            "Reconciliation status for all transactions processed through various channels and payment systems.",
            "Summary of customer service cases including type, priority, resolution time, and customer satisfaction.",
            "Activity log of the fraud detection system including alerts, investigations, and outcomes.",
            "Status report on document processing including uploads, verifications, and pending approvals.",
            "Detailed report on payment processing including volumes, values, channels, and processing times.",
            "Summary of account maintenance activities including modifications, limit changes, and status updates.",
            "Comprehensive report on standing orders including execution status, failures, and modifications.",
            "Analysis of direct debit transactions including successful collections, rejections, and returns.",
            "Statistics on card activation including activation rates, methods, and time from issuance to activation."
        };
        
        for (int i = 0; i < reportNames.length; i++) {
            Report report = new Report();
            report.setReportId("RPT" + String.format("%04d", i + 1));
            ...
            // Generate last generated date (some might be null to represent never generated)
            if (random.nextDouble() < 0.9) { // 90% chance of having a last generated date
                LocalDateTime now = LocalDateTime.now();
                LocalDateTime lastGenerated = now.minusDays(random.nextInt(30));
                report.setLastGenerated(lastGenerated);
            }
            
            report.setCreatedBy(createdBy[random.nextInt(createdBy.length)]);
