...
public class DummyData {
	...
	static {
		...
		for (i = 0; i < 40; i++) {
			...
			transaction.setId(String.valueOf(i + getRandomInt(0, 9999)));
			transaction.setStatus(getTransactionStatus().getName());
			transaction.setType(Transaction.Type.values()[random.nextInt(Transaction.Type.values().length)].getName());
			transaction.setSourceAccount(getCompany());
			transaction.setDestinationAccount(getIBAN());
			transaction.setAmount(getAmount());
			transaction.setCurrency("USD");
			transaction.setReference("REF-" + i);
			transaction.setDescription("Transaction " + i);
			transaction.setTimestamp(getPastDate(90).atStartOfDay());
